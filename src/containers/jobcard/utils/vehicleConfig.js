...
const vehicleConfig = {
  ...
  getBrandsByType(type) {
    const brandObj = this.VEHICLE_TYPE_BRAND.filter(item => item.type === type)[0];
    return brandObj ? brandObj.brands : [];
  },
  getModels(type, brand) {
    const brandObj = this.VEHICLE_BRAND_MODEL.filter(item => item.type === type)[0];
    if (brandObj) {
      const modelObj = brandObj.brands.filter(item => item.brand === brand)[0];
      return modelObj ? modelObj.models : [];
    }
    return [];
  },
  getConfigs(type, brand, model) {
    const brandlObj = this.VEHICLE_MODEL_CONFIGURATION
      .filter(item => (item.type === type && item.brand === brand))[0];
    if (brandlObj) {
      const modelObj = brandlObj.models.filter(item => item.model === model)[0];
  ...
  getLoadingByRoadApplication(roadApp) {
    const roadApps = this.ROADAPPLICATION_LOADING_TYRESIZE.filter(f => f.road === roadApp).map(v => v.loading)[0] || [];
